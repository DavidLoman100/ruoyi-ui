...
## 接口总览
- `POST /voice/template/save` 保存音色模板（含多角色映射）
...
- 请求类型：`SaveTemplateReqDTO`

```json
{
  "templateId": null,
  "templateName": "播客双人对话模板",
  "templateDescription": "甲乙角色音色配置",
  "mappings": [
    {
      "roleName": "甲",
    ...
    {
      "roleName": "乙",
      "speakerCode": "zh_female_xiaohe_uranus_bigtts",
      "contextText": "活泼"
    }
  ]
}
...
说明：
- `templateId` 为空表示新增；非空表示编辑。
- `mappings` 支持 1~N 条映射。
- 编辑时明细采用“全量替换”策略。

### SaveTemplateReqDTO 字段
- `templateId` Long，模板ID
- `templateName` String，模板名称（必填）
- `templateDescription` String，模板描述
- `mappings` `List<TemplateRoleMappingReqDTO>`（必填）

### TemplateRoleMappingReqDTO 字段
- `roleName` String，角色名称（必填）
- `speakerCode` String，音色编码（AUDIO_VOICE_CODE，必填）
...
- 返回：`Response<List<VoiceOptionResDTO>>`

## 4. 查询模板列表
- 路径：`GET /voice/template`
- 返回：`Response<List<VoiceTemplateResDTO>>`
