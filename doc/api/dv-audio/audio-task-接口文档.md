...
| 重试失败片段 | POST | `/tasks/{taskId}/retry` | `/audio/tasks/{taskId}/retry` | 仅重跑失败片段 |
| 全量重跑任务 | POST | `/tasks/{taskId}/retry-all` | `/audio/tasks/{taskId}/retry-all` | 重跑全部片段 |
...
- 说明：仅处理当前状态为 `FAILED` 的片段；成功片段会复用并最终重新合并。
- 返回：`Response<String>`，`data` 示例：`"已提交失败片段重试"`
...
- 返回：`Response<String>`，`data` 示例：`"片段文案已保存，请手动触发全量重跑"`
...
- 返回：`Response<String>`，`data` 示例：`"片段顺序已保存，请手动触发全量重跑"`
...
- 返回：`Response<String>`，`data` 示例：`"片段已删除，请手动触发全量重跑"`

## 8. 全量重跑任务

- 方法：`POST`
- 路径：`/audio/tasks/{taskId}/retry-all`
- 说明：重跑全部片段并重新合并，覆盖任务结果。
- 返回：`Response<String>`，`data` 示例：`"已提交全量重跑"`

## 9. 重跑单个片段

- 方法：`POST`
- 路径：`/audio/tasks/{taskId}/segments/{segmentId}/retry`
- 说明：仅请求该片段豆包重跑并更新该片段音频，不触发最终合并；任务状态会置为待全量重跑（`FAILED`）。
- 返回：`Response<String>`，`data` 示例：`"已提交单片段重跑"`
...
2. 文本按模板角色标记分段，推荐严格格式（角色标记单独一行）：
   ...
   第三段内容
   ```
3. 编辑/重排/删除片段后，任务状态会重置为 `CREATED`；前端统一在最终确认后调用一次 `retry-all` 进行任务级重合。
