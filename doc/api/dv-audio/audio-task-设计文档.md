...
核心接口：
- 创建任务、任务详情、片段列表
- 片段编辑/重排/删除、单片段重跑、重试失败片段、全量重跑、取消任务
- 结果查询、下载地址、分页查询、删除任务
...
- 文本要求按“角色标记独占一行 + 内容多行”的结构切段，不依赖冒号。
- 推荐输入示例：
  ```text
  [甲]
  第一段内容
...
- `retry`：只处理失败片段，成功片段复用历史产物。
- `retry-all`：重跑全部片段，覆盖任务结果。
- `segments/{segmentId}/retry`：只重跑单片段，更新片段音频，不触发最终合并。
- “单段音色不满意但状态为成功”的业务场景，使用任务级 `retry-all` 重新合成。
...
- 该状态下允许继续编辑片段，也允许后续触发单片段重跑或 `retry-all`。
